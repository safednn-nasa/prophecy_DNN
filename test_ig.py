...
def tf_testing_2():
    ...
    with tf.Session() as sess:
        ...
        #plt.subplot(1,11,1)
        #plt.imshow(im_data.reshape(28,28), cmap="Greys")
        #plt.xticks([],[])
        #plt.yticks([],[])
        #plt.title("Original image", fontsize=8)
        
        # Plot them
        #th = max(np.abs(np.min(result)), np.abs(np.max(result)))
        #plt.subplot(1,11,1+1)
        #plt.imshow(result.reshape(28,28), cmap="seismic", vmin=-1*th, vmax=th)
        #plt.xticks([],[])
        #plt.yticks([],[])
        ...
        #result = exp0.eval(feed_dict)
        #print('EXP0:')
        #print(result)
        #exs = result
        # Plot them
        #th = max(np.abs(np.min(exs)), np.abs(np.max(exs)))
        #plt.subplot(1,11,1+0)
        #plt.imshow(exs.reshape(28,28), cmap="seismic", vmin=-1*th, vmax=th)
        #plt.xticks([],[])
        #plt.yticks([],[])
        #plt.title("Exp. for ("+str(i-1)+")", fontsize=20)
        #plt.tight_layout()
        #plt.show()

        #result = exp1.eval(feed_dict)
        #print('EXP1:')
        #print(result)
        #exs = result
        # Plot them
