...
def parse_architecture_and_hyperparams(jsonFile):
    ...
    for layer in model['config']:
        ...
        if layerType.lower().startswith("conv"):
            ...
            layerTypeList.append('activation_' + layer['config']['activation'])
    ...
    print("ActivationList:",activationTypeList)
...
def conv_layer_forward_ineff(input, filters, biases, stride=1, padding=1, keras=False):
    ...
    print("output reshaped", out.shape)
    return out;
...
def sym_conv_layer_forward(input, filters, b, stride=1, padding=1, keras=False):
    ...
    h_x = np.int64(h_x)
    w_x = np.int64(w_x)
    print("out_dims:",(h_out,w_out,n_filters,h_x,w_x))
    ...
    print("Sym output shape:", out.shape)
    out = out.reshape(h_out, n_filters, w_out, h_x, w_x)
    print("Sym output reshaped", out.shape)
    #May need reshaping
    return out
...
def do_all_layers_keras_coeffs(inputNumber, outDir):
    global symInput, convWeightMatrix, denseWeightMatrix, prob, pulpInput, embedInputMatrix
    #print(inputNumber, labelMatrix[inputNumber])
    ...
    print("layerTypeList:",layerTypeList)
    for layerType in layerTypeList:
        if layerType.lower().startswith("embedding"):
            ...
            for i,l in enumerate(pulpInput):
                for j,x in enumerate(l):
                    ...
                    #lowBound and upBound removed
                    pulpInput[i][j] = pulp.LpVariable(name, cat='Continuous')
        ...
        if layerType.lower().startswith("conv"):
            ...
            activationType = activationTypeList[activationIndex].lower()
            print("activationType", activationType)
            if activationType == 'relu':
                np.set_printoptions(threshold=np.nan)
                ...
                print("embedInput:{}".format(embedInputMatrix.shape))
                ...
                for i in range(dims2[0]):
                    for j in range(dims2[1]):
                        for k in range(dims2[2]):
                            ...
                            #ADD BIAS!!!
                            bias = temp[i][j][k] - sum([symInput[i][j][k][x][y]*embedInputMatrix[x][y][0] for y in range(dims2[4]) for x in range(dims2[3])])
    ...
    if True:
        # F(x) + bias <= 0.49
        # F(x) <= 0.49 - bias
        bias = temp[0][0][maxIndex] - sum([symInput[0][0][maxIndex][x][y]*embedInputMatrix[x][y][0] for y in range(dims[1]) for x in range(dims[0])])

        l = [symInput[0][0][maxIndex][x][y]*pulpInput[x][y] for y in range(dims[1]) for x in range(dims[0])]

        #if "Attack Constraint" in prob.constraints.keys():
        #    del prob.constraints["Attack Constraint"]
        prob += (pulp.lpSum(l) <= (-bias + 0.49)), "Attack Constraint"
        #Coeffs, coeffs*input
        #if maxIndex != labelMatrix[inputNumber]:
        #    print("Error, correct label is", labelMatrix[inputNumber])
        #print("Starting LP Solving for Attack for label {}".format(attackIndex))
    prob.solve()
    prob.writeLP("TextModel.lp")
...
do_all_layers_keras_coeffs(0, './textual_models')

#TO DO, Change the LP attack to greater than 0.5 or less than 0.5.
