...
def do_all_layers_keras_coeffs(inputNumber, outDir, collect=True, attack=True):
    global symInput, convWeightMatrix, denseWeightMatrix, prob, pulpInput, constCounter
    #print(inputNumber, labelMatrix[inputNumber])
                    ...
                    for i in range(dims2[0]):
                        for j in range(dims2[1]):
                            for k in range(dims2[2]):
                                ...
                                if greater_than:
                                    prob += (pulp.lpSum(l) >= -bias),str(constCounter)
                                else:
                                    prob += (pulp.lpSum(l) <= -bias),str(constCounter)
                                constCounter += 1
    ...
    if collect and attack:
        maxIndex = attackNumber
        for attackIndex in range(symInput.shape[2]):
            ...
            prob += (pulp.lpSum(l) >= (-bias_max + bias_attack)), "Attack Constraint {} gt {}".format(maxIndex, attackIndex)
            #Coeffs, coeffs*input
            #if maxIndex != labelMatrix[inputNumber]:
            #    print("Error, correct label is", labelMatrix[inputNumber])
...
for i in range(1,2**constCounter):
    ...
    for j in reversed(range(len(binarystr))):
        if binarystr[j] == '1':
